...
export default function Carousel({ carouselType, cardIds }: CarouselProps) {
  ...
  const top = useBreakpointValue(
    carouselType === 'services' ? { base: '50%' } : { base: '39%', md: '39%' },
  );
  ...
  const Slide = ({ key, type, id }: SlideProps) => {
    return (
      ...
      >
        {type === 'services' && <ServiceCard type={type} id={id} />}
      </HStack>
    );
  };
  ...
  return (
    <Box
      ...
      width={'full'}
      //overflow={"hidden"}
      px={carouselType === 'services' ? { base: 12 } : { base: 4, xl: '97px' }}
      sx={{
        '.slick-dots': {
